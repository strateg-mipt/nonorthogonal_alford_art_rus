...
Свяжем вертикальную ось скважины с направлением оси Z системы координат и применим преобразование Фурье по z к исходным уравнениям. Значения искомых величин для каждой гармоники $\mathrm{e}^{ikz}$ в плоскости поперечного сечения скважины аппроксимируем системой базисных функций $N_j(x,y)$ на сетке конечных элементов \cite{Zienkiewicz2000}:
%\begin{equation}
%\begin{split}
%\mathbf{u}(x,y,k,\omega) & = \mathbf{N}_u(x,y) \mathbf{U}^{(j)}(k,\omega) e^{\ii(kz-\omega t)} \\
%\phi(x,y,k,\omega) & = \mathbf{N}_{\phi}(x,y) \mathbf{\Phi}^{(j)}(k,\omega) e^{\ii(kz-\omega t)}
...
\end{equation}
где матрицы $\mathbf{K}_1$, $\mathbf{K}_2$, $\mathbf{K}_3$, $\mathbf{M}$, $\mathbf{P}$ формируются из значений объёмных и поверхностных интегралов в уравнениях \eqref{var_eq_solid} и \eqref{var_eq_fluid} на элементах, а $ \hat{\mathbf{U}}$ состоит из значений искомых величин $\mathbf{U}^{(j)}$ и $\psi^{(j)}$ в узлах. 
