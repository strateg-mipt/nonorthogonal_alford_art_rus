...
\end{tabularx} 
\begin{flushleft}
* here $\theta_1^o,\theta_2^o$ и $\theta_1^n,\theta_2^n$ corresponds to orthogonal and non-orthogonal Alford rotation result. $E_{rel}$ is a ratio of non-diagonal components energy to full energy.
\end{flushleft}
...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{|X|rr|rr|r|rr|}
		...
		\hline	Bakken Shale ($15.00 \times 10.00$) &  41.2 & 25.7 & -48.8 & -32.0  & 32.3 & 22.1 & 20.0\\
	...
	\end{tabularx} 
	\begin{flushleft}
		* здесь $\theta_1^o,\theta_2^o$ и $\theta_1^n,\theta_2^n$ соответствуют результатам, полученным ортогональной и неортогональной версией алгоритма. ¬еличина $E_{rel}$ обозначает отношение энергии недиагональных компонент матрицы измерений к полной энергии.
	\end{flushleft}
...
The semi-analytical finite element method used for independent calculation the eigenvectors of dipole modes and further comparison with Alford rotation results. Following the methodology in \cite{Zharnikov2015}, we obtain eigensolution for pressure field for each of dipole modes for several frequencies within the considered range. As the initial signal have a continuous spectral density, we used for comparison an eigenvector corresponded to the frequency with maximum amplitude in spectrum of signal. The figures \ref{fig:bs10_8_HTI45} and \ref{fig:cs15_10_HTI45} show the comparison results for Bakken Shale and Cotton Shale. The observed non-orthogonality of SAFE eigenvectors fits the results of rotation at a low frequencies. We also observe a confident match of polarization directions and orthogonality of eigenvectors at high frequencies.
...
\begin{figure}[h]
	...
	\begin{tabular*}{1\textwidth}{c|cc|cc|}
		...
		\begin{minipage}{0.02\textwidth}
			\rotatebox{90}{\footnotesize \textit{ƒипольна€ мода 1}} 
		\end{minipage}&
		...
		%& & & & \\
		\begin{minipage}{0.02\textwidth}
			\rotatebox{90}{\footnotesize \textit{ƒипольна€ мода 2}} 
		\end{minipage} &
		\begin{minipage}{0.22\textwidth}
		...
		\end{minipage} \\ 
		& \footnotesize Ќ„‘, 3.29 к√ц & \footnotesize ¬„‘, 5.53 к√ц & \footnotesize Ќ„‘, 3.29 к√ц & \footnotesize ¬„‘, 5.53 к√ц \\ \cline{2-5}
	\end{tabular*}
	\renewcommand{\arraystretch}{1.0}
	\footnotesize
	\begin{tabular*}{\textwidth}{@{\extracolsep{\fill} }crccc}
		& 						 	& \tikz \draw (0,0) -- (1cm,0);  	& \tikz \draw[dashed] (0,0) -- (1cm,0);  	& \tikz \draw[dashdotted] (0,0) -- (1cm,0); \\
		& –езультаты обработки 		& \textit{ортогональный} 			& \textit{ортогональный} 					& \textit{неортогональный}    			\\
		& Alford rotation:			& \textit{без фильтрации}		 	& \textit{с фильтрацией} 					& \textit{с фильтрацией} 	\\
	\end{tabular*}
	\renewcommand{\arraystretch}{1.0}
	\normalsize
	\caption{—равнение результатов обработки фильтрованных данных измерений в породе Bakken Shale и направлений пол€ризации двух дипольных мод, полученными SAFE. «десь Ќ„‘ и ¬„‘ обозначают тип примененной к исходным данным фильтрации (низкочастотна€ и высокочастотна€ соответственно), значение частоты соответствует отображенному цветом решению SAFE дл€ собственного вектора дипольной моды.}
...
\begin{figure}[h]
	...
	\begin{tabular*}{1\textwidth}{c|cc|cc|}
		...
		\end{minipage}\\
		& \footnotesize Ќ„‘, 3.03 к√ц & \footnotesize ¬„‘, 7.17 к√ц & \footnotesize Ќ„‘, 3.03 к√ц & \footnotesize ¬„‘, 7.17 к√ц \\ \cline{2-5}
	\end{tabular*}
	\renewcommand{\arraystretch}{1.0}
	\footnotesize
	\begin{tabular*}{\textwidth}{@{\extracolsep{\fill} }crccc}
	...
	\renewcommand{\arraystretch}{1.0}
	\normalsize
	\caption{—равнение результатов обработки фильтрованных данных измерений в породе Cotton Valey Shale и направлений пол€ризации двух дипольных мод, полученными SAFE. «десь Ќ„‘ и ¬„‘ обозначают тип примененной к исходным данным фильтрации (низкочастотна€ и высокочастотна€ соответственно), значение частоты соответствует отображенному цветом решению SAFE дл€ собственного вектора дипольной моды.}
